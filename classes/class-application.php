...
class Application {
	...
	public function __construct() {
		$this->plugin_name         = 'combidesk-twinfield';
		$this->plugin_product_name = __( 'Twinfield', $this->plugin_name );
		...
		$this->description = __( 'Synchronize your WooCommerce orders automatically to Twinfield', $this->plugin_name );
		$this->run();
	}
	...
	public function woo_hpos_compatibility() {
		if ( class_exists( \Automattic\WooCommerce\Utilities\FeaturesUtil::class ) ) {
			\Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility( 'custom_order_tables', 'combidesk-twinfield/combidesk-twinfield.php', true );
		}
	}
	...
	function add_combidesk_page() {
		...
		$menu_slug  = 'combidesk-twinfield-settings';
		...
		echo '
		  <h1>' . $this->title . '</h1>
			...
			<ul>
				...
				<li>' . sprintf( __( 'Usable in combination with our %s.', $this->plugin_name ), '<a href="https://combidesk.com/integration/mollie-twinfield">' . sprintf( __( 'Mollie %s integration', $this->plugin_name ), $this->plugin_product_name ) . '</a>' ) . '</li>
				<li>' . __( 'Separate item code for Shipping and Costs.', $this->plugin_name ) . '</li>
		  ...
		  <form class="combidesk__cta-form" action="https://combidesk.com/combipackageinstall/woocommerce-twinfield" method="GET" target="_BLANK">
		  ...
		  <form class="combidesk__cta-form" action="https://www.wolterskluwer.com/nl-nl/solutions/twinfield-accounting/twinfield-boekhouden-probeer-nu" method="GET" target="_BLANK">
			...
			<input type="hidden" name="email_address" value="' . esc_attr( $email_address ) . '" />
			<input type="submit" value="' . sprintf( __( 'No %s account yet?', $this->plugin_name ), $this->plugin_product_name ) . '" />
		  </form>';
	...
	function load_combidesk_plugin_style( $hook ) {
		if ( $hook != 'toplevel_page_combidesk-twinfield-settings' ) {
			return;
		}
